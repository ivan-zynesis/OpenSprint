/**
 * OPSP Rebuild Assess Workflow Template
 *
 * Skill and command templates for assessing rebuild impact
 * when driver specs change — traces invalidated ADRs and
 * produces minimal operator re-evaluation prompts.
 */
import type { SkillTemplate, CommandTemplate } from '../types.js';

export function getRebuildAssessSkillTemplate(): SkillTemplate {
  return {
    name: 'opensprint-rebuild-assess',
    description: 'Assess rebuild impact when driver specs change — traces invalidated decisions, cascades re-evaluation, and minimizes operator questions by reusing unaffected decisions.',
    instructions: REBUILD_ASSESS_INSTRUCTIONS,
    license: 'MIT',
...
export function getOpspRebuildAssessCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSP: Rebuild Assess',
    description: 'Assess rebuild impact when driver specs change',
    category: 'Workflow',
    tags: ['workflow', 'opsp', 'rebuild', 'assessment'],
    content: REBUILD_ASSESS_INSTRUCTIONS,
  };
}

const REBUILD_ASSESS_INSTRUCTIONS = `Assess the impact of changed driver specs on existing decisions.
