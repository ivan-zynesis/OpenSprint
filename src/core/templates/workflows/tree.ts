/**
 * OPSP Tree Workflow Template
 *
 * Skill and command templates for visualizing and navigating
 * the decision tree with blast radius analysis.
 */
import type { SkillTemplate, CommandTemplate } from '../types.js';

export function getTreeSkillTemplate(): SkillTemplate {
  return {
...
export function getOpspTreeCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSP: Tree',
    description: 'Visualize the decision tree with blast radius analysis',
    category: 'Workflow',
    tags: ['workflow', 'opsp', 'decision-tree', 'visualization'],
    content: TREE_INSTRUCTIONS,
  };
}

const TREE_INSTRUCTIONS = `Visualize or regenerate the decision tree in \`opensprint/DECISION-MAP.md\`.
