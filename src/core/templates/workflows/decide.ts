/**
 * OPSP Decide Workflow Template
 *
 * Skill and command templates for recording operator decisions
 * when the agent encounters ambiguity it cannot resolve autonomously.
 */
import type { SkillTemplate, CommandTemplate } from '../types.js';

export function getDecideSkillTemplate(): SkillTemplate {
  return {
...
export function getOpspDecideCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSP: Decide',
    ...
    content: DECIDE_INSTRUCTIONS,
  };
}

const DECIDE_INSTRUCTIONS = `Record a decision in \`opensprint/ADRs/\` when genuine ambiguity requires operator judgment.
