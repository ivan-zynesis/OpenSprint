/**
 * OPSP Reexplore Workflow Template
 *
 * Initiative-scoped brainstorm and plan revision. Loads full initiative
 * context, enters explore mode, then orchestrates milestone plan changes
 * through existing opsx workflows.
 */
import type { SkillTemplate, CommandTemplate } from '../types.js';

export function getOpspReexploreSkillTemplate(): SkillTemplate {
  return {
    ...
    instructions: OPSP_REEXPLORE_INSTRUCTIONS,
    license: 'MIT',
...
export function getOpspReexploreCommandTemplate(): CommandTemplate {
  return {
    name: 'OPSP: Reexplore',
    description: 'Re-explore an initiative — brainstorm, revise milestones, update proposals',
    category: 'Workflow',
    tags: ['workflow', 'opsp', 'reexplore', 'initiative', 'planning', 'revision'],
    content: OPSP_REEXPLORE_INSTRUCTIONS,
  };
}

const OPSP_REEXPLORE_INSTRUCTIONS = `Re-explore an initiative to brainstorm and revise the milestone plan.
