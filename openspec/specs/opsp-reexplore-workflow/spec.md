# opsp-reexplore-workflow Specification

## Purpose

Define the `/opsp:reexplore` workflow skill that allows an operator to revisit an existing initiative, assess its current state against the plan, and revise the milestone plan through existing opsx workflows.
